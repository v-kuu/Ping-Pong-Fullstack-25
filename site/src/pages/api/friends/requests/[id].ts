...
export const PATCH: APIRoute = async ({ params, request, locals }) => {
    ...
    try {
        ...
        return new Response(JSON.stringify({ error: "Invalid body, expected { status: 'accepted' | 'rejected' }" }), { status: 400 });
    }
    ...
    try {
        ...
        return new Response(JSON.stringify({ success: true, status: statusAction }), {
            status: 200,
        });
    } catch (error) {
...
export const DELETE: APIRoute = async ({ params, locals }) => {
    ...
    try {
        ...
        if (result.length === 0) {
            return new Response(JSON.stringify({ error: "Request not found or cannot cancel" }), {
                status: 404, // or 403
            });
        }

        return new Response(JSON.stringify({ success: true, message: "Request cancelled" }), {
            status: 200,
        });
